...
class SaleWorkflowProcess(models.Model):
    ...
    next_run_time = fields.Integer(default="1")

    @api.onchange('next_run_time')
    def sale_auto_run_cron_update(self):
        cron = self.env.ref('sale_automatic_workflow.ir_cron_automatic_workflow_job')
        cron.interval_number = self.next_run_time
        cron.active = True
