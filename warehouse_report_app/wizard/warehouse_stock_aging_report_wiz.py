...
class StockAgingWarehouseReport(models.TransientModel):
	_name = 'stock.aging.warehouse.wout.movement'
	_description = 'Stock Aging Without Movement Report'
	...
	def print_pdf_report(self):
		...
		datas = {
			 'ids': [1],
			 'model': 'stock.aging.warehouse.wout.movement',
			 'form': data
		}
		return self.env.ref('warehouse_report_app.action_stock_aging_ware_wout_movement').report_action(self, data=datas)
...
# Location
	...
	def print_excel_report(self):
		self.ensure_one()
		[data] = self.read()
		file_path = 'Stock Aging Without Movement Report' + '.xlsx'
		workbook = xlsxwriter.Workbook('/tmp/' + file_path)
		worksheet = workbook.add_worksheet('Stock Aging Without Movement Report')
		...
		#Merge Row Columns
		TITLEHEDER = 'Stock Aging Without Movement Report' 
			...
			for warehouse in warehouse_ids:
				...
				for records in self.get_warehouse_details(data, warehouse):
					for record in records.get('product_data'):
						...
						sub_total = col_1_data + col_2_data + col_3_data + col_4_data + col_5_data
		...
		self.document = buf
		self.file = 'Stock Aging Without Movement Report'+'.xlsx'
		return {
			'res_id': self.id,
			'name': 'Files to Download',
			'view_type': 'form',
			"view_mode": 'form,tree',
			'res_model': 'stock.aging.warehouse.wout.movement',
